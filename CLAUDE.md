# CLAUDE.md — Poker Note プロジェクトルール

> このファイルは Claude が新しい会話を開始するたびに自動で読み込むプロジェクトレベルの指示書である。
...
| コマンド | 説明 |
|---|---|
| `/fix-bug` | バグ修正（TDDサイクルに従った構造化修正） |
| `/add-feature` | 機能追加（要件確認→TDD→実装→検証） |
| `/make-requirements` | 要件定義書の作成（ポーカー知識を自動注入） |
| `/code-review` | コードレビュー（品質チェックリスト準拠） |
...
| `/extract-knowledge` | 解決した問題や手順から、再利用可能なルール・スキルを抽出・保存する |
| `/evolve` | 蓄積された教訓（Instincts）を整理し、正式なルールやドキュメントに昇華させる |
